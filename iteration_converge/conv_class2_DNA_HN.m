...
%%%%%%%%%%%%%%%%%%%%%%%%%%% Learn HN
structure_epoch = 5; % overall iteration
i_epoch = 500; % training iteration
j_epoch = 3; % confidence iteration
K = 20;
nHE = 100 * 10;
...
for structure_l = 1:structure_epoch
    ...
    for j = 1:j_epoch
        ...
        for i = 1:i_epoch
            ...
            scr_HN_one_step % one weight update 
...
x = 1:100;
t1 = shadedErrorBar(x,lol(:,1)',lol_std(:,1)','lineprops','-b','patchSaturation',0.33) % single
t2 = shadedErrorBar(x,lol(:,2)',lol_std(:,2)','lineprops','-r','patchSaturation',0.33) % ensemble
hold on;
h1 = plot(x, lol(:,1),'b','LineWidth',2) %single
h2 = plot(x, lol(:,2),'r','LineWidth',2) %ensemble
