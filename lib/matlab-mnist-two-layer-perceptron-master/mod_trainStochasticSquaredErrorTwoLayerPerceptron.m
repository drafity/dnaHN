function [hiddenWeights, outputWeights, error, correctlyClassified] = mod_trainStochasticSquaredErrorTwoLayerPerceptron(activationFunction, dActivationFunction, numberOfHiddenUnits, inputValues, targetValues, epochs, batchSize, learningRate, test_inputValues, test_labels)
...
% 
    ...
    hiddenWeights = zeros(numberOfHiddenUnits,inputDimensions); %rand(numberOfHiddenUnits, inputDimensions);
    outputWeights = zeros(outputDimensions, numberOfHiddenUnits); %rand(outputDimensions, numberOfHiddenUnits);
...
%     outputWeights = rand(outputDimensions, numberOfHiddenUnits);
    ...
    for t = 1: epochs
        ...
        correct = validateTwoLayerPerceptron(activationFunction, hiddenWeights, outputWeights, test_inputValues, test_labels);
        correctlyClassified(t) = correct; 
        %plot(t, error,'*');
    end;
end